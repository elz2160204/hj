...
def get_node_topics(nodeId=nodes["收费视频"], page=1, type=types["出售"], limit=100, _=1):
    ...
    try:
        with session.get(url=url, headers=headers) as resp:
            ...
            return page, json.loads(data)
    except Exception as e:
        logger.error(f"{page} 请求异常: {e}")
        return get_node_topics(nodeId, page, type, limit, _+1)


def paqu_node_topics():
    os.makedirs("data", exist_ok=True)
    logger.info("开始执行程序")
    users = pickle.loads(bucket.get("", "user"))

    limit = 100
    uplimit = 10000
    totals = {}
    # '讨论': 10000, 数据有80w+ 太大以后下载 已下载154084 24 类型:2 第1543页 获取到 100 条信息
    dili = {'讨论': 10000, '没话找话': 10000}  # 80w+
    for name in dili:
        print(name, f"https://www.haijiao.com/api/topic/node/topics?page=1&nodeId={nodes[name]}&type=2&limit=100")
        new_user = []
        flag = True
        ...
        if posts_data:
            bucket.put_json("", f"topics {nodeId} {total} {int(time.time())}", posts_data)
        ...
        if new_user:    # users: set()
            users.update([i["userid"] for i in new_user])
            logger.info("正在添加新用户")
            for i, x in enumerate([uplimit for i in range(len(posts_data) // uplimit)]):
                logger.info(f"正在上传 {i} {uplimit}")
                bulk_insert_data(User, new_user[uplimit * i:uplimit * (i + 1)])
            logger.info("已上传完大半")
            bulk_insert_data(User, new_user[-(len(posts_data) % uplimit):])
            bucket.put("", "user", pickle.dumps(users))
        else:
            logger.info("没有新用户")
        ...
        if posts_data:
            for i, x in enumerate([uplimit for i in range(len(posts_data) // uplimit)]):
                logger.info(f"正在上传 {i} {uplimit}")
                bulk_insert_data(Topic, posts_data[uplimit * i:uplimit * (i + 1)])
            logger.info("已上传完大半")
            bulk_insert_data(Topic, posts_data[-(len(posts_data) % uplimit):])
        logger.success("任务完成")
